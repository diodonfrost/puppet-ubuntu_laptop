...
#
# The assumption is that users have UID greater than 500, got home directory and don't have shell with nologin
...
if Facter.value(:kernel) == 'Linux'
  ...
  if File.exists?('/usr/bin/getent')
    ...
    Facter::Util::Resolution.exec('/usr/bin/getent passwd').each_line do |line|
      ...
      users << user[0] if ( user[2].to_i > 500 && user[5].include?("/home") && !user[6].include?("nologin") )
    end
  else
